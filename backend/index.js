...
// so disabling the mock in production avoids the latency of external DoH HTTP requests to Google DNS.
if (process.env.NODE_ENV !== 'production' && process.env.DISABLE_DNS_MOCK !== 'true') {
  console.log('Enabling DNS-over-HTTPS (DoH) mock for MongoDB Atlas DNS issues...');
  ...
  console.log('Running in production mode. Using Node.js native DNS resolution.');
}
...
mongoose
  ...
  .then(async () => {
    ...
    try {
      ...
      if (adminCount === 0) {
        const seedEmail = process.env.ADMIN_EMAIL || 'admin@irttech.ac.in';
